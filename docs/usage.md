...
- **세 파일 모두 `--output-dir` 에 이미 있는 파일에 병합한다. 통째 교체가 아니다.**
  링크는 바뀐 것만 — 드래그한 테이블, 편집한 그룹·메모만 — 담으므로, 링크가 말하지 않은
  것은 거기 놓여 있는 배포본에서 가져온다. `layout.json` 은 **필드 단위로** 병합한다:
  옮기기만 한 테이블은 색을 유지하고, 색만 바꾼 테이블은 좌표를 유지한다.
- `layout.json` 은 무엇을 했는지 숫자로 보고한다 — `(89 tables: 53 kept, 33 updated, 3 added)`.
  병합할 배포본이 없었으면 그렇다고 말한다.
...
- `groups` 도 마찬가지로 검증 없이 그대로 쓴다 — CLI 는 검증 경계가 아니다. 실제 검증은
  뷰어의 `parseGroups` 가 로드 시점에 하고, 두 그룹이 같은 테이블을 지목하면 **먼저 쓴
  그룹이 가져간다**. `?showgroups=` 는 순수 뷰 설정이라 `from-link` 는 아예 읽지 않는다.
...
그룹핑은 이름 접두사 기준이다 — `estimate_*`, `template_*`. 의견이 아니라 출발점이라, 그룹
이름이 `Rename me 1` 식으로 붙는다.

FK 연결요소를 먼저 시도했는데 실무 스키마에서는 안 된다. 허브 테이블 하나가 전부를 한
덩어리로 잇기 때문이다. 실제 77테이블 애플리케이션이 74개짜리 그룹 하나로 나왔고, 접두사
기준으로는 11개로 갈렸다.
...
| `⌘`/`Ctrl` + 클릭, `Shift` + 클릭 | 선택에 추가·제외 *(편집 모드에서만)* |
...
| `⌘G` / `Ctrl+G` | 선택한 테이블을 그룹으로 묶기 *(편집 모드에서만)* |
| `⌘⇧G` / `Ctrl+Shift+G` | 선택이 속한 그룹 해제 — 확인 후 진행 *(편집 모드에서만)* |
| `⇧1` | 화면에 맞추기 |
| `⇧2` | 모든 컬럼 표시 |
| `⇧3` | 테이블 이름만 |
| `⇧4` | 키만 |
...
헤더의 **`Edit` 버튼**으로 켜고 끄거나, 파라미터를 직접 붙인다:
...
만들 수도, 스키마를 고칠 수도 없다. 공유 링크가 실수로 흐트러지는 것을 막기 위한 기본값이다.
...
편집 모드에서만 드래그된다. 놓는 즉시 `?positions=`에 반영된다.
...
| 클릭 대상 | 메뉴 |
|---|---|
| 빈 캔버스 | `Add memo here` / `Add table here` — 둘 다 클릭한 지점에 생성. 편집분이 있으면 `Discard schema edits` |
| 테이블 | `Connect`(관계 종류를 고른 뒤 상대 테이블 클릭) + 색상 팔레트 + 그룹 |
...
편집 모드에서는 테이블 상세 패널이 폼으로 바뀐다. 스키마가 담고 있는 것은 전부 여기서
고칠 수 있다 — 테이블 이름·코멘트, 컬럼(이름·타입·기본값·코멘트·`PRIMARY KEY`·`NOT NULL`),
외래 키·유니크·체크 제약, 인덱스. 각 섹션은 접히고, 헤더에 그 섹션에 추가하는 버튼이 있다.
빈 캔버스 메뉴의 `Add table here` 는 테이블을 만들고 바로 그 폼을 연다.

편집분은 `?schemaedits=` 로 간다. `schema.json` 에는 아무것도 쓰지 않고, 어디로도 전송하지
않는다. 파라미터에는 **실제로 고친 테이블만** 실리므로 링크가 짧게 유지되고, 원래 모양으로
되돌리면 그 테이블은 파라미터에서 다시 빠진다. 전부 버리려면 빈 캔버스 메뉴의
`Discard schema edits`.

참조는 가리키는 대상을 따라간다. 다이어그램이 없는 것을 가리키게 되는 일은 없다:

| 편집 | 따라오는 것 |
|---|---|
| 테이블 이름 변경 | 그 테이블을 가리키던 모든 외래 키. 고정 위치·색상·그룹 소속도 함께 따라간다 |
| 테이블 삭제 | 그 테이블을 가리키던 외래 키가 함께 삭제됨 |
| 컬럼 이름 변경 | 같은 테이블의 제약·인덱스, 그리고 그 컬럼을 가리키던 다른 테이블의 외래 키 |
| 컬럼 삭제 | 위와 동일. 컬럼이 하나도 남지 않은 제약·인덱스는 삭제됨 |

일부러 건드리지 않는 것이 둘 있다. `CHECK` 제약의 식은 자유 형식 SQL 이라 컬럼 이름을
바꿔도 **치환하지 않는다** — 직접 고쳐야 한다. 그리고 복합 기본 키의 컬럼 순서는 테이블의
컬럼 순서를 따른다. 컬럼별 `Primary key` 체크박스로는 다른 순서를 표현할 수 없다.

뷰어 전체가 편집된 스키마를 읽으므로 **`Export` 도 편집된 DDL 을 뱉는다** — MySQL,
PostgreSQL, YAML 전부.

편집은 다이어그램을 재배치하지 않는다. 화면 위치·선택·뷰포트가 그대로 유지되고, 테이블이
행 하나만큼 커지면 **바로 아래 걸리는 테이블만** 내려가 자리를 만든다. 다시 줄어들어도
올라오지 않는다(빈칸이 남는다). 전체를 다시 배치하고 싶으면 툴바의 `Tidy up`.

#### 두 테이블 연결하기

키를 가질 쪽 테이블을 `Ctrl`/`Cmd` + 우클릭해 관계 종류를 고른 뒤, **연결할 테이블을
클릭한다.** 진행 중에는 무엇이 무엇을 기다리는지 배지로 알려주고, `Esc` 나 빈 캔버스
클릭으로 취소된다.

종류는 스키마에 저장되는 값이 아니라 **링크를 어떻게 만들지에 대한 지시**다. 저장되는
것은 외래 키뿐이고, 다이어그램이 그리는 카디널리티는 거기서 파생된다:

| | |
|---|---|
| `many : 1` | 먼저 클릭한 테이블에 외래 키 |
| `1 : 1` | 같은 키 + 그 컬럼에 `UNIQUE` (ONE_TO_ONE 의 유일한 표현) |
| `1 : many` | 두 테이블을 바꾼 것과 같다 — 어느 쪽이 many 인지는 컬럼을 가진 쪽이 정한다 |
| `many : many` | 단일 키로는 표현 불가라 **조인 테이블**을 만든다. 양쪽으로 외래 키 + 둘을 묶은 기본 키. 두 테이블의 중간에 고정된다 |

참조 컬럼은 이미 있는 `<대상>_<키>` 또는 `<끝의 s 를 뗀 대상>_<키>` 컬럼이 있으면 그것을
쓰고, 없으면 `<대상>_<키>` 컬럼을 새로 만든다(타입은 참조하는 키에 맞춤). 어느 쪽이었는지는
토스트로 알려준다. 양쪽 다 가리킬 기본 키가 필요하고, 없으면 지어내지 않고 그렇다고 알려준다.
...
12색 고정. 전부 `@crowfoot/ui` 의 기존 디자인 토큰에서 가져왔다.
...
FK 로 자동 추론하지 않는다 — 언제나 사람이 명시적으로 묶는다. **한 테이블은 최대 한 그룹에만
속한다.** 상자는 매 렌더 멤버들의 bounding box 에서 파생되므로, 테이블을 공유하면 두 상자가
반드시 교차하고 한쪽을 끌면 다른 쪽까지 변형된다. `erd arrange` 도 공유된 테이블을 두 컬럼에
놓을 수 없다. 결제·정산처럼 도메인이 겹치는 테이블은 **하나의 그룹을 골라서** 표현한다.

> 0.4.2 이전에는 한 테이블이 여러 그룹에 속할 수 있었다. 그때 만든 `groups.json` 이나 링크도
> 그대로 열린다: 그 테이블을 **먼저** 지목한 그룹이 가져가고, 자기 몫이 하나도 남지 않은
> 그룹은 버려진다. 반면 `erd arrange` 는 그런 계획을 해석하지 않고 **거부한다** — 계획은
> 물려받은 것이 아니라 지금 쓰는 것이기 때문이다.
...
- **그룹에 속한 테이블 우클릭** — `Remove from "이름"` 으로 그 테이블을 속한 그룹에서 뺀다.
- **옮기기** — 테이블을 선택하고 선택 패널의 `Move to`. 어떤 그룹에 들어가는 것은 곧 원래
  그룹에서 나오는 것이고, 그래서 비게 된 그룹은 버려진다.
...
한 테이블은 한 그룹에만 속하므로, 두 모드 모두 테이블마다 정확히 한 줄이다.
...
```
layout.json  +  ?positions= (링크)  >  자동 배치(ELK)
```

메모와 그룹도 같은 구조다. `memos.json` / `groups.json` 이 기준이고 `?memos=` /
`?groups=` 는 **링크가 바꾼 것만** 담는다 — 이름 바꾼 그룹, 추가한 메모, 지운 메모.
그래서 사이드카를 다시 배포하면 링크를 들고 있는 사람에게도, 본인이 건드리지 않은
것에 한해 반영된다.
...
링크의 편집이 지금 배포된 사이드카와 다른 버전에서 작성됐다면, 조용히 적용하지 않고
알린다. `?base=` 가 그 편집이 어떤 문서를 기준으로 만들어졌는지 기록하고, 어긋나면
사라진 참조를 짚어준다 — 스키마에 없는 그룹 멤버 테이블, 지울 대상이 없는 삭제 같은
것. 편집 자체는 그대로 적용된다.

> 링크의 내용은 **링크에만** 있다. 브라우저에는 배포본 사본 하나만 남고, 그것도 위
> 안내를 더 자세히 쓰는 데만 쓰인다. 팀에 보여주려면 링크를 공유하거나 아래 방법으로
> 사이드카 파일에 고정한다.
...
crowfootLayout.reset()   // 링크에서 레이아웃 편집을 빼고 새로고침
...
| `tableNames` | string[] | ✅ | 멤버 테이블 이름. **한 테이블은 한 그룹에만 속한다**: 앞 항목이 이미 가진 테이블을 뒤 항목이 또 지목하면 뒤 항목이 그것을 잃고, 그래서 비면 항목 자체가 버려진다. 빈 배열이면 항목 전체가 버려진다. 같은 항목 안의 중복 이름은 하나로 합쳐진다. |
| `color` | string | | 팔레트 키. 목록 밖 값은 무시된다. |
...
| `schemaedits` | 압축 JSON | 스키마 편집분. 통째로 교체된 테이블 + 삭제된 이름. | replace |
| `edit` | `1` \| `true` | 편집 활성화. 없으면 읽기 전용. | push |

### 인코딩

`positions` / `colors` / `memos` / `groups` / `schemaedits` 는 **deflate 압축 + URL-safe base64**(`+`→`-`,
...
- `schemaedits` 는 **실제로 고친 테이블만** 담고, 삭제한 테이블은 이름만 담는다. 원래
  모양으로 되돌려 놓으면 그 테이블은 파라미터에서 다시 빠진다.
...
편집(`positions`/`colors`/`memos`/`groups`/`schemaedits`)은 드래그 한 번마다 히스토리를
채우면 안 되기 때문에 의도적으로 나눴다. `edit` 은 편집분이 아니라 모드라서 `push` 다 —
뒤로가기로 편집 모드를 빠져나온다.
...
**내 브라우저에서만 배치가 유지된다**
아직 링크에만 있는 상태다. [배치를 팀에 고정하는 3가지 방법](#배치를-팀에-고정하는-3가지-방법) 참고.
...
**`from-link` 를 돌렸더니 `layout.json` 에 테이블이 몇 개밖에 안 남았다**
0.4.1 이하의 결함이다 — 링크에 실린 테이블만 남기고 나머지 좌표를 지웠다. **0.4.2 부터는
`--output-dir` 의 기존 `layout.json` 에 병합**하고, `(89 tables: 53 kept, 33 updated,
3 added)` 처럼 무엇을 했는지 숫자로 보고한다. 이미 유실된 파일은 git 에서 되돌린 뒤
0.4.2 이상으로 다시 돌린다.
