...
| 테이블 | `Connect to`(다른 테이블로 외래 키 연결) + 색상 팔레트 |
...
외래 키·유니크·체크 제약, 인덱스. 빈 캔버스 메뉴의 `Add table here` 는 테이블을 만들고
바로 그 폼을 연다.
...
키를 가질 쪽 테이블을 `Ctrl`/`Cmd` + 우클릭하고 `Connect to` 에서 대상 테이블을 고른다.
외래 키는 대상의 기본 키를 가리키고, 참조 컬럼은:

1. 이미 있는 `<대상>_<키>` 또는 `<끝의 s 를 뗀 대상>_<키>` 컬럼이 있으면 그것을 쓰고,
2. 없으면 `<대상>_<키>` 컬럼을 새로 만든다. 타입은 참조하는 키에 맞춘다.

어느 쪽이었는지는 토스트로 알려준다. 대상에 기본 키가 없으면 가리킬 것이 없으므로,
없는 키를 지어내지 않고 그렇다고 알려준다.
