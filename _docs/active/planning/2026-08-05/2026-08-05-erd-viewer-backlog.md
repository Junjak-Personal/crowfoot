...
- 🔴 **~~Playwright 는 이미 저장소에 있다~~ — 2026-08-06 `444f80d` 에서 `@liam-hq/e2e` 를
  삭제했다.** upstream 앱 전용이라 포크가 안 쓰는 패키지였고, 정리 시점에 "나중에 다시
  만든다" 를 감수하고 지웠다. **이 항목은 이제 하네스 구축부터 시작한다** — 비용이 올라갔다
