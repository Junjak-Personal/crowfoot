...
| **Table grouping** | Gather tables into named groups, drawn as a dashed tinted box behind them. A table belongs to at most one group — the box is derived from its members' bounds, so a shared table would force two boxes to cross. `Move to` in the selection panel moves a table between groups. Dragging a group's label moves its members together. One toolbar control switches between **group view** (boxes drawn, sidebar sectioned by group) and **single view** (no boxes, the plain alphabetical list). Shipped with the build in `groups.json`. |
